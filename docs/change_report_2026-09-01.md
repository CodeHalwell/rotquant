...
| Rotation training (PR review follow-up) | `rotquant/train_rotation.py`, `rotquant/patch.py`, `rotquant/calibrate.py` | The gate can remove the calibration damping ridge (`damp_frac`, plumbed as `patch_model(hessian_damp_frac=...)`, recorded on `CalibrationResult`). | Codex P2 on the accounting head: `finalize` adds `damp_frac * mean(diag(H)) * I`, so scoring through it measures `tr(E Sigma E^T) + lambda ||E||^2`. The harness collects with `damp_frac=0.0`, so no recorded run is affected, and the ridge inflates both sides of the ratio: measured at most 1.009x on the score and no decision reversed in 120 synthetic trials. Fixed because the function's contract is the deployed error, not because a result moved. |
| Rotation training (PR review follow-up) | `rotquant/train_rotation.py`, `rotquant/patch.py` | `hessian_reconstruction_error` and the gate take an `activation_mean`; `patch_model` passes it for `mean`/`length_mean` bias correction, so both arms are scored on `H - mu^T mu`. | Codex P2 on the merge head: `apply_mean_bias_correction` folds the mean error into the bias, so scoring `H` ranks a component the deployment cancels. `configs/qwen35_4b_w4a8_e8_trials_cuda.yaml` pairs `objective: hessian` with `bias_correction: mean`, and the ranking reverses in 16 of 40 synthetic seeds. |
...
**Follow-up (PR review).** The first version of this fix still let the write
pass pack rows: a write of `written` rows kept its last `recent` rows in fp16
and packed the rest, and the ageing pass then packed the range from
`pending_start` to `new_length − recent`, which includes those same rows. A
write longer than the window (chunked prefill, speculative or batched decode)
therefore quantized its leading rows twice, compounding the error. The Codex
review of the second head raised this. The write pass now places every
incoming row in the fp16 recent tier (`recent_window=written`, rotated fp16
storage as before) and only the ageing pass packs rows, exactly once, when they
leave the window; sink rows are still decided by absolute position. The
evaluator (`_evaluate_kv_cache`) decodes one token per step, so no recorded
number changes. `test_chunked_tiered_writes_pack_each_row_exactly_once` and
`test_chunked_tiered_writes_decide_sinks_by_absolute_position` require a
