...
| llama.cpp / Metal | Experimental GGUF v1 loader, tied vocabulary, Metal weight kernels, and the Qwen3.5 frozen mixed 3.25-bpv persistent KV cache are implemented in the pinned fork. | Yes | Build with `scripts/build_rotquant_llama_cpp.sh`; use the custom GGUF artifact with flash attention enabled. |
